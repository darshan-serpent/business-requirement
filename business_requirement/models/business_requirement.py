...
class BusinessRequirement(models.Model):
    ...
    def message_post(self, body='', subject=None,
        ...
        subject = None
        if self._context.get('default_model') == 'business.requirement'\
                and self._context.get('default_res_id'):
            br_rec = self.pool.get(self._context.get('default_model')).\
                browse(self._context['default_res_id'])
            subject = 'Re: %s-%s' % (br_rec.name, br_rec.description)
        return super(BusinessRequirement, self).message_post(
            body='', subject=subject, subtype=None, parent_id=False,
